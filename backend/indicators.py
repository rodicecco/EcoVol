...
class Composite:
    def __init__(self, indicators:list):
        ...
        self.upper = 2
        self.lower = 1
        self.from_date = '2022-05-16 00:00'
    ...
    def indicator(self):
        ...
        for ind in self.indicators:
            ind.indicator()
            data_lis.append(ind.export_to_comp[['signal']])
        self.composite_data = pd.concat(data_lis, axis=1)
        self.composite_data.columns = [ind.__class__.__name__ for ind in self.indicators]
        

        self.composite_data['sum'] = self.composite_data.sum(axis=1)
        self.composite_data.loc[self.composite_data['sum']>=self.upper, 'signal'] = -1
        self.composite_data.loc[self.composite_data['sum']<=self.lower, 'signal'] = 0
        self.composite_data['signal'] = self.composite_data['signal'].fillna(1)

        indicator_data_lis = []
        for ind in self.indicators:
            indicator_data_lis.append(ind.indicator_data[ind.indicator_handout])
        self.indicator_data = pd.concat(indicator_data_lis, axis=1)
        self.indicator_data = self.indicator_data.merge(self.composite_data[['signal', 'sum']], 
                                                        left_index=True, right_index=True, 
                                                        how='left')
        
        spy_data = self.get_spy_data()
        self.indicator_data = self.indicator_data.merge(spy_data[['close']], 
                                                        left_index=True, right_index=True, 
                                                          how='left', suffixes=('', '_SPY'))
        self.indicator_data = self.indicator_data.dropna()
    ...
    def plot_indicator(self, from_date=None):
        ...
        all_data = all_data.loc[from_date:]

        # Define the ratios: 
        # [3] for the main chart, [0.5] for the signals, [1] for composite, and [1] for each indicator
        view_ratios = [2, 0.5, 0.5] + [0.5] * len(self.indicators)

        # Calculate the total number of rows
        num_rows = len(self.indicators) + 3

        fig = Figure(figsize=(10, 12)) # Increased height to accommodate more subplots
        # Use fig.subplots to create all axes at once with the ratios applied
        axes = fig.subplots(nrows=num_rows, ncols=1, sharex=True, 
                            gridspec_kw={'height_ratios': view_ratios})

        # axes is now a list where:
        # axes[0] is the large top chart
        # axes[1] is the signal fill_between
        # axes[2] is the composite signal
        # axes[3:] are your individual indicators

        # --- Plotting ---

        # Axis 1 (Top - 3x size)
        ax1 = axes[0]
        ax1.set_title('SPY Close Price with Composite and Indicators')
        ax1.plot(all_data.index, all_data['close'], label='SPY Close Price', color='blue')
        ax1.grid(False)

        # Axis 2 (Small - 0.5x size)
        ax2 = axes[1]
        ...
        # Axis 3 (Normal - 1x size)
        ax3 = axes[2]
        ax3.plot(all_data.index, all_data['sum'], label='Composite Signal', color='purple')
        ax3.plot(all_data.index, [self.upper]*len(all_data), label='Upper Threshold', color='black', linestyle='--')
        ax3.plot(all_data.index, [self.lower]*len(all_data), label='Lower Threshold', color='black', linestyle='--')
        ax3.grid(False)
        # ... rest of ax3 code ...

        # Indicators (Each 1x size)
        for i, ind in enumerate(self.indicators):
            ax = axes[i + 3]
            for col in ind.indicator_handout:
                ax.plot(all_data.index, all_data[col], label=col)
            ax.plot(all_data.index, [ind.upper]*len(all_data), label='Upper Threshold', color='black', linestyle='--')
            ax.plot(all_data.index, [ind.lower]*len(all_data), label='Lower Threshold', color='black', linestyle='--')
            ax.grid(False)
...
class VolSpread:
    ...
    def __init__(self, symbol='SPY', from_date='2022-05-16 00:00'):
        self.symbol = symbol
        self.upper = 0.8
        self.lower =0.7
        ...
        self.raw_data = None
        self.avg_window = 5
        self.indicator_handout = ['vol_ratio', 'ratio_avg']
        self.autocorr_vol = None
    ...
    def get_data(self):
        ...
        query = f'''SELECT * FROM intraday WHERE symbol ='{self.symbol}'
                    AND datetime > '{self.from_date}';'''
        
        with admin.Database('', []).connection() as conn:
    ...
    def vix_vol(self):
        ...
        #from_date = datetime.fromisoformat(self.from_date)
        #from_date = from_date.strftime("%Y-%m-%d")

        query = f'''SELECT * FROM historical WHERE symbol = 'VIX1D.INDX' 
                    AND date > '{self.from_date}';'''
        with admin.Database('', []).connection() as conn:
            vix_data = pd.read_sql(query, conn)
    ...
    def indicator(self):
        ...
        all_data['ratio_avg'] = all_data['vol_ratio'].rolling(window=self.avg_window).mean()
        ...
        self.export_to_comp = all_data[['vol_ratio', 'ratio_avg', 'signal']]
        self.autocorr_vol = AutoCorrVol(all_data['actual_vol'])
    ...
    def plot_indicator(self, from_date=None):
        ...
        (ax1, ax4, ax3) = fig.subplots(3, 1, gridspec_kw={'height_ratios': [3, 0.5, 1]})
        ...
        # Instead of fig.legend(loc='lower center'...)
        fig.legend(
            loc='lower center', # Place that point at 50% width and 5% height
            ncol=6,                     # Spread items horizontally
            fontsize=8                # Adds a border to separate it from the background
        )

        # Adjust the layout to make room for the legend at the top
...
class AutoCorrVol:
    def __init__(self,symbol='SPY',  max_lag = 10):
        
        self.symbol = symbol
        self.max_lag = max_lag
        self.from_date = '2022-05-16'
        self.lower = 0
        self.upper = 0.15
        self.indicator_handout = ['actual_vol']
        self.start_time = '09:30'
        self.end_time = '16:00'
        self.raw_data = None
    ...
    def indicator(self):            
        vol = self.actual_vol()
        spy_data = self.get_spy_data()
        rolling_autocorr  = vol.rolling(window=self.max_lag).apply(lambda x: x.autocorr(lag=1))
        self.indicator_data = rolling_autocorr
        self.indicator_data = self.indicator_data.merge(spy_data[['close']], left_index=True, right_index=True, how='left')
        self.indicator_data.loc[self.indicator_data['ann_vol']>self.upper, 'signal'] = 0
        self.indicator_data.loc[self.indicator_data['ann_vol']<self.lower, 'signal'] = -1
        self.indicator_data['signal'] = self.indicator_data['signal'].fillna(1)

        self.from_date = vol.index.min().strftime("%Y-%m-%d")
    ...
    def plot_indicator(self, from_date=None):
        ...
        (ax1, ax3, ax2) = fig.subplots(3, 1, gridspec_kw={'height_ratios': [2,0.5, 1]})

        ax1.set_title('SPY Close Price with Rolling Autocorrelation of Actual Volatility')
        ax1.plot(all_data['close'], label='SPY Close Price', color='blue')
        ax1.grid(False)

        ax3.fill_between(all_data.index, 1, color='yellow', alpha=0.5, where=all_data['signal']==1)
        ax3.fill_between(all_data.index, 1, color='green', alpha=0.5, where=all_data['signal']==0)
        ax3.fill_between(all_data.index, 1, color='red', alpha=0.5, where=all_data['signal']==-1)
        ax3.grid(False)

         # Axis 3 (Normal - 1x size)
        ax2.plot(all_data.index, all_data['actual_vol'], label='Rolling Autocorrelation of Actual Volatility', color='orange')
        ax2.plot(all_data.index, [self.upper]*len(all_data), label='Upper Threshold', color='black', linestyle='--')
        ax2.plot(all_data.index, [self.lower]*len(all_data), label='Lower Threshold', color='black', linestyle='--')
        ax2.grid(False)
...
class VixSpread:
    def __init__(self, from_date='2022-05-16 00:00'):
        ...
        self.lower = 8
        self.avg_window = 5
        self.indicator_handout = ['vvix_vix_diff', 'dif_avg']
    ...
    def indicator(self):
        ...
        self.indicator_data = all_data
        self.export_to_comp = all_data[['vvix_vix_diff', 'dif_avg', 'signal']]
